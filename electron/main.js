...
ipcMain.on('start-terminal-process', (event, command, args, cwd) => {
  if (terminalProcess) {
    terminalProcess.kill();
  }
  terminalProcess = spawn(command, args, { cwd: cwd || process.cwd(), shell: true });
...
ipcMain.on('kill-terminal-process', () => {
  if (terminalProcess) {
    terminalProcess.kill();
    terminalProcess = null;
  }
